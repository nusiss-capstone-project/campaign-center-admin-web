...
export default function AdminTaskCreatePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const [groups, metricData, operatorData] = await Promise.all([
          fetchTaskGroups(),
          fetchDataMetrics(),
          fetchDataMetricOperators(),
        ]);
        ...
        const group = normalizeTaskGroupRow(
        ...
        if (!group || !isDraftStatus(group.status)) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/admin/task-group/${groupId}/tasks/${taskId}`);
    } catch (err) {
      ...
      <Card className="gap-0 border-white/10 bg-zinc-900/50 py-0 text-zinc-100">
        ...
        {loading ? (
          <CardContent className="px-6 py-6">
            <p className="text-sm text-zinc-400">Loading reference data...</p>
          </CardContent>
        ) : (
